...
def adjust_learning_rate(optimizer,
    ...
    for param_group in optimizer.param_groups:
        if encoder_lr_ratio != 1.0 and "backbone." in param_group["name"]:
            # backbone_lr=param_group['lr']
            param_group['lr'] = (now_lr - min_lr) * encoder_lr_ratio + min_lr
            backbone_lr = param_group['lr']
            if "v_id" in param_group["name"] or "xm_id" in param_group["name"] or "linear_ID_KV" in param_group["name"]:
                param_group['lr']=min(2*((now_lr - min_lr) * encoder_lr_ratio + min_lr),now_lr)
            if "self.conv_proj_v" in param_group["name"]:
                param_group['lr'] = min(2 * ((now_lr - min_lr) * encoder_lr_ratio + min_lr), now_lr)
                #print(param_group["name"]+"lr:"+ str(param_group['lr']))
        else:
