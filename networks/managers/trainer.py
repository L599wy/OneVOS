...
#from dataloaders.train_datasets_bl30k import DAVIS2017_Train, YOUTUBEVOS_Train, StaticTrain, BL30K_Train,TEST
from dataloaders.train_datasets import DAVIS2017_Train, YOUTUBEVOS_Train, StaticTrain,TEST,MOSE_Train,LVOS_Train
import dataloaders.video_transforms as tr

from utils.meters import AverageMeter
from utils.image import label2colormap, masked_image, save_image
from utils.checkpoint import load_network_and_optimizer, load_network, save_network,load_backbone
...
class Trainer(object):
    def __init__(self, rank, cfg, enable_amp=True):
        ...
        self.model_backbone = self.model.backbone
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TRAIN_RESUME:
            ...
            if cfg.PRETRAIN_FULL:
                ...
                self.model_backbone, removed_dict = load_backbone(
                    self.model_backbone, cfg.MODEL_ENCODER_PRETRAIN, self.gpu)
                if len(removed_dict) > 0:
                    self.print_log('Remove {} from pretrained model.'.format(
                        removed_dict))
                self.print_log(
    ...
    def prepare_dataset(self):
        ...
        if 'lvos' in cfg.DATASETS:
            train_mose_dataset = LVOS_Train(
                root=cfg.DIR_LLVOS_Train,
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_YTB,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
    ...
    def sequential_training(self):
        ...
        for i in range(cfg.DATA_SEQ_LEN - 2):
            frame_names.append('Curr{}'.format(i + 1))

        seq_len = len(frame_names)-1
        ...
        while step < cfg.TRAIN_TOTAL_STEPS:
            ...
            for frame_idx, sample in enumerate(train_loader):
                ...
                if step % cfg.TRAIN_LR_UPDATE_STEP == 0:
                    now_lr,backbone_lr = adjust_learning_rate(
                        optimizer=optimizer,
                        base_lr=cfg.TRAIN_LR,
                ...
                if self.enable_amp:
                    with torch.cuda.amp.autocast(enabled=True):
                        assert not torch.isnan(ref_imgs).any(), print("ref_imgs,appear nan")
                        assert  not torch.isnan(prev_imgs).any(), print("prev_imgs,appear nan")
                ...
                for idx in range(seq_len):
                    ...
                    # skip ref frame
                    #prev_label=all_labels[(idx) * bs:(idx + 1) * bs].detach()
                    now_label = all_labels[(idx+1) * bs:(idx + 2) * bs].detach()
                ...
                if self.rank == 0:
                    ...
                    if step % cfg.TRAIN_TBLOG_STEP == 0:
                        ...
                        if seq_len == 1:
                            self.process_log(ref_imgs, all_f[-2], all_f[-1],
                                             ref_labels, prev_labels, now_label,
                                             now_pred, boards, running_losses,
                                             running_ious, now_lr, backbone_lr,step)
                        else:
                            self.process_log(ref_imgs, all_f[-2], all_f[-1],
                    ...
                    if step % cfg.TRAIN_LOG_STEP == 0:
                        strs = 'I:{}, LR:{:.6f},backbone_LR:{:.6f}, T:{:.1f}({:.1f})s, Obj:{:.1f}({:.1f})'.format(
                            step, now_lr, backbone_lr, batch_time.val,
                        ...
                        for idx in range(seq_len):
                            strs += ', {}: L {:.3f}({:.3f}) IoU {:.1f}({:.1f})%'.format(
                                frame_names[idx+1], running_losses[idx].val,
    ...
    def process_log(self, ref_imgs, prev_imgs, curr_imgs, ref_labels,
                    ...
                    running_losses, running_ious, now_lr,backbone_lr, step):
        cfg = self.cfg
        ...
        if cfg.TRAIN_IMG_LOG or cfg.TRAIN_TBLOG:
            ...
            if cfg.TRAIN_TBLOG:
                ...
                self.tblogger.add_scalar('Backbone LR', backbone_lr, step)
                ...
                for key in boards['image'].keys():
                    ...
                    for seq_step in range(len(tmp)):
                        self.tblogger.add_image('S{}/'.format(seq_step) + key, tmp[seq_step].detach().cpu().numpy(),
                                                step)
                for key in boards['scalar'].keys():
                    tmp = boards['scalar'][key]
                    for seq_step in range(len(tmp)):
                        self.tblogger.add_scalar('S{}/'.format(seq_step) + key, tmp[seq_step].detach().cpu().numpy(),
                                                 step)
