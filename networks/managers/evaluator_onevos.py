...
from dataloaders.eval_datasets import YOUTUBEVOS_Test, YOUTUBEVOS_DenseTest, DAVIS_Test, EVAL_TEST,MOSE_Test,LLVOS_Test
...
class Evaluator(object):
    def __init__(self, cfg, rank=0, seq_queue=None, info_queue=None,ck=None):
        ...
        self.process_pretrained_model(ck)

        self.prepare_dataset()

    def process_pretrained_model(self,ck):
        cfg = self.cfg
    ...
    def prepare_dataset(self):
        ...
        ckpt_use=os.path.splitext(os.path.basename(self.ckpt))[0]
        ...
        elif cfg.TEST_DATASET == 'davis2017_test':
            resolution = 'Full-Resolution' if cfg.TEST_DATASET_FULL_RESOLUTION else '480p'
            self.result_root = os.path.join(cfg.DIR_EVALUATION,
                                            cfg.TEST_DATASET, eval_name,
                                            'Annotations', resolution)
        ...
        with torch.no_grad():
            for seq_idx, seq_dataset in enumerate(self.dataset):
                ...
                for frame_idx, samples in enumerate(seq_dataloader):
                    ...
                    # print("cfg.MODEL_ENGINE: ",cfg.MODEL_ENGINE)
                    for aug_idx in range(len(samples)):
                        ...
                        #print("cur_img_shape:",current_img.shape)
                        ...
                        if frame_idx == 0:
                            if current_label is None:
                                ...
                                current_label = torch.zeros(B, 1, H, W).cuda(
                                    self.gpu, non_blocking=True).float()
                            ...
                            _current_label = F.interpolate(
                                ...
                                mode="nearest")
                            engine.process_reference_frame(current_img,
                                                           _current_label,
                                                           frame_step=0,
                                                           obj_nums=obj_nums)
                            ...
                            engine.match_propogate_one_frame(current_img, self.prev_img)
                            if self.ref_obj:
                                engine.update_mem()
                                self.ref_obj=False
